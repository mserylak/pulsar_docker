...
# fftw-2.1.5
ENV PATH $PATH:$PSRHOME/fftw-2.1.5/install/bin
ENV LD_LIBRARY_PATH $LD_LIBRARY_PATH:$PSRHOME/fftw-2.1.5/install/lib
ENV C_INCLUDE_PATH $C_INCLUDE_PATH:$PSRHOME/fftw-2.1.5/install/include
WORKDIR $HOME
RUN wget http://www.fftw.org/fftw-2.1.5.tar.gz
RUN tar -xvf fftw-2.1.5.tar.gz -C $PSRHOME
WORKDIR $PSRHOME/fftw-2.1.5
RUN ./configure --prefix=$PSRHOME/fftw-2.1.5/install --enable-float --enable-shared --enable-threads F77=gfortran CFLAGS=-fPIC FFLAGS=-fPIC
RUN make \
#    && make check \
    && make install \
    && make clean
...
# cfitsio (not necessary as we are installing it system-wide with apt-get)
...
# hdf5
#ENV PATH $PATH:$PSRHOME/hdf5-1.8.17/hdf5/bin
#ENV LD_LIBRARY_PATH $LD_LIBRARY_PATH:$PSRHOME/hdf5-1.8.17/hdf5/lib
#ENV C_INCLUDE_PATH $C_INCLUDE_PATH:$PSRHOME/hdf5-1.8.17/hdf5/include
#WORKDIR $HOME
#RUN wget http://www.hdfgroup.org/ftp/HDF5/current/src/hdf5-1.8.17.tar.bz2
#RUN tar -xvf hdf5-1.8.17.tar.bz2 -C $PSRHOME
#WORKDIR $PSRHOME/hdf5-1.8.17
...
# h5check
#ENV PATH $PATH:$PSRHOME/h5check-2.0.1/install/bin
#WORKDIR $HOME
#RUN wget https://www.hdfgroup.org/ftp/HDF5/tools/h5check/src/h5check-2.0.1.tar.gz
#RUN tar -xvf h5check-2.0.1.tar.gz -C $PSRHOME
#WORKDIR $PSRHOME/h5check-2.0.1
